            ...
            while (true) {
                try {
                    ...
                    if (Packet_Receive != null) {
                        ...
                        String ss = String.valueOf(Packet_Receive.getData());

                        // 判断数据是否合法
                        InetSocketAddress address = (InetSocketAddress) Packet_Receive.getSocketAddress();
//                        String the_ip = address.getHostName().substring(0,SERVER_IP.length());
                  /*      Log.w("ip",address.getHostName() );
                        Log.w("11",the_ip);
                        Log.w("tongguoyanzheng",String.valueOf(the_ip.equals(SERVER_IP)));
                        // 判断是否是调度服务器的ip
                        if (!the_ip.equals(SERVER_IP)) {
                            continue;
                        }*/
                        ...
                        int length = (ss.length()-1)*2;
                        if (length > 0)
                            Log.w("1111", Byte2String.bytesToHexStringwithlength(Packet_Receive.getData(),18));
                            mConnectListener.OnReceiverCallBack(18, Packet_Receive.getData());
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    Log.e("Show", e.toString());
